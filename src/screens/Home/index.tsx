...
const Home = () => {
  ...
  const [myBooks] = useState<IBookData[]>(myBooksData);
  const [categories] = useState<ICategoriesData[]>(categoriesData);
  ...
  function renderHeader(profileData: IProfileData) {
    return (
      <View
        style={{
          flex: 1,
  ...
  function renderMyBookSection(books: IBookData[]) {
    const renderItem = ({item, index}: {item: IBookData; index: number}) => {
      return (
        <TouchableOpacity
          style={{
            flex: 1,
  ...
  function renderCategoryHeader() {
    const renderItem = ({item}: {item: ICategoriesData}) => {
      return (
        <TouchableOpacity
  ...
  function renderCategoryData() {
    ...
    const selectedCategoryBook = categories.find(
      (item) => item.id === selectedCategory,
    );

    if (selectedCategoryBook) {
      books = selectedCategoryBook.books;
    }

    const renderItem = ({item}: {item: IBookData}) => {
      return (
  ...
  return (
    <SafeAreaView style={{flex: 1, backgroundColor: COLORS.black}}>
      ...
      {/* Body Section */}
      <ScrollView style={{marginTop: SIZES.radius}} horizontal={false}>
        {/* Books Section */}
